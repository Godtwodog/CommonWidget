...
//    private boolean isCurrentMonth = true; //判断是否为当前月份
...
//            }
            ...
            if (mStartDate != null && mEndDate == null && !mStartDate.equals(mEndDate)) {
...
//        drawMonthDayLabels(canvas);
    ...
    float startX = 0.0F;
    float startY = 0.0F;
    float startRawX = 0.0F;
    float startRawY = 0.0F;
...
//        }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.i("dengxs", "开始:"+startX + "," + startY + "," + startRawX + "," + startRawY);
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                float stopRawX = event.getRawX();
                float stopRawY = event.getRawY();
                Log.i("dengxs", "结束:"+stopX + "," + stopY + "," + stopRawX + "," + stopRawY);
                Log.i("dengxs", "X偏移:"+Math.abs(startX - stopX));
                Log.i("dengxs", "Y偏移:"+Math.abs(startY - stopY));

                if (Math.abs(startX - stopX) <= 60 && Math.abs(startY - stopY) <= 60){
                    //单击
                    Log.i("dengxs", "单选");
                }else {
                    Log.i("dengxs", "拖拽");
